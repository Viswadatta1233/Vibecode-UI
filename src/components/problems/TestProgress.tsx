...
const TestProgress: React.FC<TestProgressProps> = ({ results, percentage, passedCount: propPassedCount, totalCount: propTotalCount, status }) => {
    ...
    return (
      <div className="mt-6 p-4 bg-leetcode-gray-50 dark:bg-leetcode-gray-800 rounded-lg border border-leetcode-gray-200 dark:border-leetcode-gray-700">
        <div className="flex items-center space-x-2">
          <Clock className="h-4 w-4 sm:h-5 sm:w-5 text-leetcode-gray-400" />
          <span className="text-sm sm:text-base text-leetcode-gray-600 dark:text-leetcode-gray-300">Waiting for test results...</span>
        </div>
      </div>
    );
  }
  ...
  const getStatusIcon = (result: TestResult, status: string) => {
    ...
    if (status === 'Running' && result.output === '' && !result.error && result.passed === false) {
      return <Clock className="h-4 w-4 sm:h-5 sm:w-5 text-blue-500 animate-spin" />;
    }
    if (result.error) {
  ...
  return (
    <div className="mt-6 space-y-4">
      {/* Progress Bar */}
      <div className="bg-leetcode-gray-50 dark:bg-leetcode-gray-800 rounded-lg border border-leetcode-gray-200 dark:border-leetcode-gray-700 p-3 sm:p-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 gap-1">
          <h3 className="text-base sm:text-lg font-semibold text-leetcode-gray-900 dark:text-white">
            Test Results
          </h3>
          <span className="text-xs sm:text-sm text-leetcode-gray-600 dark:text-leetcode-gray-300">
            {finalPassedCount}/{finalTotalCount} passed
          </span>
        </div>
        
        {/* Overall Score Display */}
        <div className="mb-4 p-2 sm:p-3 bg-leetcode-gray-100 dark:bg-leetcode-gray-700 rounded-lg">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <span className="text-xs sm:text-sm font-medium text-leetcode-gray-700 dark:text-leetcode-gray-300">
              Overall Score:
            </span>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs sm:text-sm text-leetcode-gray-500 dark:text-leetcode-gray-400">
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 gap-2">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium text-sm sm:text-base text-leetcode-gray-900 dark:text-white">
                  Test Case {index + 1}
                </span>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
              <div>
                ...
                <div className="mt-1 p-2 bg-leetcode-gray-100 dark:bg-leetcode-gray-800 rounded font-mono text-xs break-all">
              ...
              {result.error && (
                <div className="lg:col-span-2">
                  ...
                  <div className="mt-1 p-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded font-mono text-xs text-red-700 dark:text-red-300 break-all">
                    {result.error}
                  </div>
                </div>
              )}
